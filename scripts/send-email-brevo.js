...
/**
 * Script d'envoi d'emails via Brevo API (Sendinblue)
 * Usage: node scripts/send-email-brevo.js <to> <template> [data_json]
 *
 * Exemples:
 *   node scripts/send-email-brevo.js user@example.com welcome '{"name":"Jean"}'
 *   node scripts/send-email-brevo.js user@example.com email-verification '{"otp":"123456","name":"Jean"}'
 */
...
const BREVO_SENDER_EMAIL = process.env.BREVO_SENDER_EMAIL || 'no-reply@notifications.ansut.ci';
...
const emailTemplates = {
    ...
    html: (data) => `
      ...
      <body>
        <div class="header">
          ...
          <p>Mon Toit - Inscription</p>
        </div>
        <div class="content">
          <h2>Bonjour ${data.name || 'utilisateur'} !</h2>
          <p>Voici votre code de vérification pour compléter votre inscription sur Mon Toit :</p>
          <div class="otp-code">${data.otp}</div>
          <p style="text-align: center; color: #6b7280;">
            <strong>Ce code expire dans ${data.expiresIn || '5 minutes'}</strong>
          </p>
          <div class="warning">
  ...
  'test': {
    subject: 'Email de test - Mon Toit (Brevo)',
    html: (data) => `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div class="box">
          <h1>✅ Email de test Brevo réussi !</h1>
          <p>L'envoi d'emails via Brevo fonctionne correctement.</p>
...
async function sendEmail(to, template, data = {}) {
  // Vérifier la clé API
  if (!BREVO_API_KEY) {
    console.error('❌ Erreur: BREVO_API_KEY non définie');
    console.log('Exportez la variable d\'environnement: export BREVO_API_KEY=votre_clé');
    process.exit(1);
  }
  ...
  if (!to || !template) {
    ...
    console.log('Usage: node send-email-brevo.js <to> <template> [data_json]');
  ...
  const payload = {
    ...
    htmlContent: emailTemplate.html(templateData),
    tags: ['mon-toit', template]
  };

  console.log('\n📧 Envoi d\'email via Brevo...');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log(`De:     ${BREVO_SENDER_NAME} <${BREVO_SENDER_EMAIL}>`);
  console.log(`À:      ${to}`);
