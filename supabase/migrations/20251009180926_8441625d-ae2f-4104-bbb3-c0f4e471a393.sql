-- ============================================================================
-- PHASE 1: SÉCURITÉ CRITIQUE - Protection données personnelles
-- ============================================================================
...
-- 2. Fonction pour vérifier si un admin a activé la 2FA
...
-- 3. Renforcer get_verifications_for_admin_review avec obligation 2FA
...
BEGIN
  ...
  -- NOUVEAU: Vérifier 2FA obligatoire
  IF NOT public.admin_has_2fa_enabled(auth.uid()) THEN
    INSERT INTO public.admin_audit_logs (
      admin_id, action_type, target_type, target_id, notes
    ) VALUES (
      auth.uid(), 'sensitive_data_access_blocked_no_2fa', 'user_verification', auth.uid(),
      'Admin attempted to access sensitive data without 2FA'
    );
    
    RAISE EXCEPTION 'Two-factor authentication required. Enable 2FA in profile settings.';
  END IF;
  
  -- Logger l'accès avec 2FA
  INSERT INTO public.sensitive_data_access_log (
    requester_id, target_user_id, data_type, access_granted, relationship_type, metadata
  ) VALUES (
    auth.uid(), NULL, 'verification_admin_queue', true, 'super_admin',
    jsonb_build_object('action', 'admin_queue_accessed', 'has_2fa', true, 'timestamp', now())
  );
  ...
  SELECT uv.user_id, p.full_name, p.user_type, p.city,
         ...
         uv.admin_review_notes, uv.admin_reviewed_at, uv.admin_reviewed_by,
...
-- 5. Renforcer RLS profiles - Bloquer accès cross-user au téléphone
...
SELECT 
  sdal.id, sdal.requester_id, p_req.full_name as requester_name,
  sdal.target_user_id, p_tgt.full_name as target_name,
  sdal.data_type, sdal.access_granted, sdal.relationship_type, sdal.metadata, sdal.accessed_at,
  CASE 
...
LEFT JOIN public.profiles p_req ON p_req.id = sdal.requester_id
LEFT JOIN public.profiles p_tgt ON p_tgt.id = sdal.target_user_id
ORDER BY sdal.accessed_at DESC;

GRANT SELECT ON public.sensitive_data_access_monitoring TO authenticated;

-- RLS sur sensitive_data_access_log
...
-- 7. Fonction détection accès suspects
...
BEGIN
  IF NOT has_role(auth.uid(), 'super_admin'::app_role) THEN
    RAISE EXCEPTION 'Only super admins can detect suspicious patterns';
  END IF;
