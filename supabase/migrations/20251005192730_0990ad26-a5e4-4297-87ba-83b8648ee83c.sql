...
-- Replace them with explicit auth checks that are more reliable

-- Drop the unsafe policies
DROP POLICY IF EXISTS "Deny unauthenticated access to profiles" ON public.profiles;
DROP POLICY IF EXISTS "Deny unauthenticated access to verifications" ON public.user_verifications;

-- Create safer policies that explicitly check authentication
CREATE POLICY "Block unauthenticated profile access"
ON public.profiles
FOR ALL
USING (auth.uid() IS NOT NULL AND false)
WITH CHECK (auth.uid() IS NOT NULL AND false);

CREATE POLICY "Block unauthenticated verification access"
ON public.user_verifications
FOR ALL
USING (auth.uid() IS NOT NULL AND false)
WITH CHECK (auth.uid() IS NOT NULL AND false);

-- Add comments explaining why these policies exist
