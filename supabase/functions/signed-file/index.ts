...
serve(async req => {
    ...
    try {
        ...
        if (!fileName || fileName === "signed-file") {
            return new Response(
                JSON.stringify({
                    error: "signedfilename path parameter is required",
                }),
                {
                    status: 400,
                    headers: {
        ...
        const fileUrl = `${CRYPTONEO_API_URL}/sign/getSignedFile/${encodeURIComponent(fileName)}`
        ...
        if (!fileRes.ok) {
            ...
            return new Response(
                JSON.stringify({
                    error: `Failed to fetch signed file (${fileRes.status}): ${errText}`,
                }),
                {
