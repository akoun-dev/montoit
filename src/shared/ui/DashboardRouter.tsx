...
export default function DashboardRouter() {
  ...
  useEffect(() => {
    const checkRedirect = async () => {
      ...
      // Check if user is an agency agent (has active entry in agency_agents table)
      // This check takes precedence over the user_type check
      try {
        ...
        if (!agentError && agentData) {
          console.log('DashboardRouter - User is an agency agent, redirecting to /agent/dashboard');
          navigate('/agent/dashboard', { replace: true });
          return;
        }
      } catch (error) {
        console.error('DashboardRouter - Error checking agency agent status:', error);
        // Continue with normal routing if check fails
      }
