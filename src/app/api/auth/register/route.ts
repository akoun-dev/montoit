...
export async function POST(req: NextRequest) {
    try {
        const { email, password, firstName, lastName, phone, role, method } =
            await req.json()
        ...
        if (method === "email") {
            ...
            if (!userPayload) {
                    ...
                    return NextResponse.json(
                        {
                            error:
                                authError?.message ||
                                "Impossible de créer le compte Supabase",
                        },
                        { status: 400 }
                    )
                }
                ...
                if (insertError || !insertedProfile) {
                    await admin.auth.admin
                        .deleteUser(authUserData.user.id)
                        .catch(() => {})
                    throw (
                        insertError ||
                        new Error("Impossible de créer le profil applicatif")
                    )
                }
            ...
            if (!emailResult.success) {
                console.warn(
                    `[Register] Email send failed for ${normalized}, but OTP stored. Code: ${otpCode}`
                )
            }
        ...
        if (method === "sms") {
            ...
            if (
                existingUser &&
                existingUser.first_name === "Temp" &&
                existingUser.last_name === "User"
            ) {
            ...
            if (!smsResult.success) {
                console.warn(
                    `[Register] SMS send failed for ${phone}, but OTP stored. Code: ${otpCode}`
                )
            }
        ...
        return NextResponse.json(
            { error: "Méthode d'inscription non supportée" },
            { status: 400 }
        )
    } catch (error) {
