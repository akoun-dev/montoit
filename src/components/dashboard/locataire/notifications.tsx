...
// Maps generic notification actionUrls to role-specific dashboard section IDs
  ...
  const handleMarkAsRead = async (notif: NotificationItem) => {
    try {
      ...
      if (notif.actionUrl) {
        ...
        const section = mapActionUrl(notif.actionUrl, role)
        if (notif.entityId) {
