...
export default function RemindersPage() {
  ...
  const loadExpiringLeases = async () => {
    ...
    try {
      const { data: contracts } = await (supabase as any)
        .from('lease_contracts')
        .select('id, property_id, tenant_id, end_date, properties(title), tenant:profiles(first_name)')
        .eq('owner_id', user.id)
        .eq('status', 'active')
  ...
  const handleSendReminder = async (reminderId: string) => {
    try {
      // Simuler l'envoi (à remplacer par l'appel à l'Edge Function)
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              ...
              {/* Type de rappel et Canal - en grille */}
